...
def build_path_metric_checkpoints(
    ...
    while i <= 20:
        checkpoint_counts.add(round(initial_node_count * i / 20))
        i += 1
...
def simulate_random_node_failure(
    ...
    initial_step = measure_attack_step(working_graph, initial_node_count, removed_fraction=0.0, is_path_metric_checkpoint=True)
    ...
    while working_graph.number_of_nodes() > 0:
        ...
        step = measure_attack_step(working_graph, initial_node_count, removed_fraction, is_path_metric_checkpoint, removed_item)
